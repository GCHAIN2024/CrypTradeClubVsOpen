...
setlocal
set FORCE_FLAG=0
set SLN_PATH="C:\Dev\GCHAIN2024\CrypTradeClubVsOpen"
...
for /f "tokens=2 delims=," %%a in ('wmic process where "commandline like '%%%ROOT_PATH%%%\\%CrypTradeClubVsOpen%\\Server\\bin\\Publish\\%\\%\\%\\server.exe%'" get ProcessId /format:csv ^| findstr [0-9]') do (
    taskkill /pid %%a /f
)
IF ERRORLEVEL 1 (
